    ...
    async send(notification, msg, monitorJSON = null, heartbeatJSON = null) {
        try {
            if (heartbeatJSON == null) {
                const title = "Uptime Kuma Distributed Alert";
                const monitor = {
                    type: "ping",
                    url: "Uptime Kuma Distributed Test Button",
                };
                return this.postNotification(notification, title, msg, monitor, "trigger");
            }

            if (heartbeatJSON.status === UP) {
                const title = "Uptime Kuma Distributed Monitor ✅ Up";
                return this.postNotification(notification, title, heartbeatJSON.msg, monitorJSON, "recovery");
            }

            if (heartbeatJSON.status === DOWN) {
                const title = "Uptime Kuma Distributed Monitor 🔴 Down";
    ...
    async postNotification(notification, title, body, monitorInfo, eventAction = "trigger") {
        ...
        const options = {
            ...
            data: {
                ...
                entity_display_name: "Uptime Kuma Distributed Alert: " + monitorInfo.name,
                routing_key: notification.pagerdutyIntegrationKey,
                entity_id: "Uptime Kuma Distributed/" + monitorInfo.id,
            },
        };

        const baseURL = await setting("primaryBaseURL");
        if (baseURL && monitorInfo) {
            options.client = "Uptime Kuma Distributed";
